...
MyInstance* construct()
{
  ...
  my->effect = effectv_init(onedRegister, 0, 0);
...
void update(void* instance)
{
  ...
  if(inst->in_doublebuff->number != my->old_doublebuf) {
      doublebuf = inst->in_doublebuff->number;
      my->old_doublebuf = inst->in_doublebuff->number;
  }
