...
MyInstance* construct()
{
  ...
  my->effect = effectv_init(TransFormRegister, 0, 0);
...
void update(void* instance)
{
  ...
  if (inst->in_transform->number != my->old_transform) {
      if(inst->in_transform->number < 6)
	  transform = inst->in_transform->number;
      my->old_transform = inst->in_transform->number;
    }
