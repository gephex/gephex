...
MyInstance* construct()
{
  ...
  my->effect = effectv_init(dizzyRegister, 0, 0);
...
void update(void* instance)
{
  ...
  if (inst->in_phase->number != my->old_phase) {
      phase = inst->in_phase->number;
      my->old_phase = inst->in_phase->number;
    }

  if (inst->in_zoom->number != my->old_zoom) {
      zoomrate = inst->in_zoom->number;
      my->old_zoom = inst->in_zoom->number;
    }
